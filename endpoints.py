...
def get_endpoints(filter):
    ...
    try:
        ...
        if AXLRequest.status_code == 401:
            result = "Authentication error"
        else:
            result = "Success"
    except requests.exceptions.ConnectionError:
        DeviceNames = list()
        result = "Failed to connect to the CUCM AXL Service"
    return DeviceNames, result
...
def get_IPaddress(filter):
    ...
    try:
        ...
        if SOAPRequest.status_code == 401:
            result = "Authentication error"
        else:
        ...
        result = "Failed to connect to the CUCM RisPort70 Service"
    return IP, result
